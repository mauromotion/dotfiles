...
- In =early-init.el= package.el is disabled for elpaca (the package manager), and some basic initial UI settings are configured.
- Then in =init.el= in order, there's the start up performance function, the whole elpaca installation and setup, and a check that loads =config.el= if present on disk, or, if not present, it will tangle this =config.org= file so that =config.el= gets generated. And then =init.el= loads the whole configuration from =config.el=.
...
(defvar mmotion/config-org
  ...
  "Absolute path to my literate Emacs config.")

(defun mmotion/auto-tangle-and-load-config ()
  "When `config.org` is saved, tangle it (and reload the result)."
  (when (and buffer-file-name
             (string= (file-truename buffer-file-name)
                      (file-truename mmotion/config-org)))
    (message "[literate] Tangling %s…" mmotion/config-org)
    (let ((org-confirm-babel-evaluate nil))
      (org-babel-tangle-file mmotion/config-org))
    (let ((generated-el (concat (file-name-sans-extension mmotion/config-org) ".el")))
      (when (file-exists-p generated-el)
        (message "[literate] Loading %s…" generated-el)
