...
(defun mmotion/minibuffer-backward-kill-alt (arg)
  "When minibuffer is completing a file name, delete up to parent
folder at word boundaries, otherwise delete a single character."
  (interactive "p")
  (if minibuffer-completing-file-name
      (let ((contents (minibuffer-contents)))
        (cond
         ;; No slash in path or at word boundary - delete to parent folder
         ((or (not (string-match-p "/." contents))
              (looking-back "\\b" 1))
          (if (string-match-p "/." contents)
              (zap-up-to-char (- arg) ?/)
            (delete-minibuffer-contents)))
         ;; In middle of word - delete single character  
         (t (delete-backward-char arg))))
    (delete-backward-char arg)))
  #+end_src
