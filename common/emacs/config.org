#+TITLE: Mauro's GNU Emacs Configuration
...
- [[#org-mode][Org-mode]]
  ...
  - [[#org-mode-file-types-associations][Org-mode file types associations]]
  - [[#org-mode-capture-templates][Org-mode Capture Templates]]
  - [[#evil-org][Evil-org]]
  - [[#toc-org][TOC-org]]
  - [[#org-babel][Org-Babel]]
  - [[#org-download][Org-download]]
  - [[#org-inline-animated-gifs][Org inline animated gifs]]
...
All the temp and backup files that Emacs generates are put into "~/.cache/emacs" instead of alongside the original files.
...
(use-package no-littering
  :demand t
  :config
  ;; Set the main Emacs directory for no-littering
...
(use-package ef-themes
  ...
  :config
  (setq modus-themes-to-toggle '(ef-elea-light ef-elea-dark))
...
(use-package doric-themes
  :demand t
  :config
  (setq doric-themes-to-toggle '(doric-coral doric-obsidian)))
  ;; (doric-themes-select 'doric-coral))
#+end_src
*** Doom-themes
#+begin_src emacs-lisp 
;;;; doom-themes
...
(with-eval-after-load 'doom-modeline
    (set-face-attribute 'mode-line nil :font "NotoSansM Nerd Font-12")
    (set-face-attribute 'mode-line-inactive nil :font "NotoSansM Nerd Font-12"))
   #+end_src
*** Mood-line
...
#+end_src
** Spacious padding
Control padding of Emacs' various elements.
...
(use-package nov
  ...
  (add-hook 'nov-mode-hook
            (lambda ()
              ;; Improved typography
              (face-remap-add-relative 'variable-pitch :family "Work Sans" :height 1.1)
              ;; Better line spacing
...
(use-package elfeed
  ...
  (setq elfeed-search-face-alist
      '((elfeed-search-unread-title-face)
        (unread (:weight bold))
        (youtube (:foreground "firebrick"))
        (blog (:foreground "forest green"))
        (gaming (:foreground "dark goldenrod"))
        (tech (:foreground "steel blue"))
        (emacs (:foreground "medium orchid"))))
...
(defun elfeed-toggle-star ()
  "Toggle the 'star' tag on the current Elfeed entry."
  (interactive)
  (elfeed-search-toggle-all 'star))

(evil-define-key 'normal elfeed-search-mode-map (kbd "t s") 'elfeed-toggle-star)
...
(use-package elfeed-org
  :init
  (setq rmh-elfeed-org-files '("~/Notes/plain_orgfiles/orgfiles/elfeed.org"))
  :config
  (elfeed-org))
...
** Org-mode Todo Keywords
  ...
  (org-todo-keywords
   ...
   #+end_src
** Org-mode file types associations
#+begin_src elisp :lexical no
...
(use-package olivetti
  ...
  :custom
  (olivetti-body-width 110)           ; set body width
...
#+end_src
*** Stylize quote blocks
#+begin_src elisp :lexical no
 (defun mmotion/set-org-quote-face (&rest _)
  "Style org quote blocks with theme-aware subtle background."
...
;; Call it after org loads
(add-hook 'org-mode-hook #'mmotion/set-org-quote-face)

;; And after theme changes (add to your existing advice)
(advice-add 'load-theme :after #'mmotion/set-org-quote-face)
(advice-add 'enable-theme :after #'mmotion/set-org-quote-face)

(add-hook 'org-mode-hook
          (lambda ()
            (font-lock-add-keywords nil
              '(("^[ \t]*\\(#\\+begin_quote\\).*$" 1 '(face org-meta-line display "❝"))
                ("^[ \t]*\\(#\\+end_quote\\).*$" 1 '(face org-meta-line display "❞"))))))
#+end_src
